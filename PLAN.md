...
- `Async\current_context()` даёт хранилище, изолированное по корутине/скоупу — это
...
| **Per-coroutine адаптеры** | оборачивают stateful-синглтоны, перенося их изменяемое состояние в `current_context()` |
...
**Выбрана: общий контейнер + изоляция состояния через `current_context()`**
...
- каждая корутина-запрос работает в собственном дочернем `Scope`, поэтому
  `current_context()` уникален для запроса;
...
`AsyncCurrentUser` — identity и кэш проверок прав в `current_context()`.
...
│   └── ScopedKey.php         # enum-ключи для current_context()
└── tests/
```
